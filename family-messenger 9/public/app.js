...
// ---- Клик мышью (десктоп) — открывает тап-меню сообщения ----
$('#messages').addEventListener('click', (e) => {
  ...
  if (row && !row.dataset.wasSwipe) openMessageMenu(row.dataset.messageId);
});
$('#image-lightbox').addEventListener('click', () => $('#image-lightbox').classList.add('hidden'));

// ---- Свайпы на телефоне: влево = ответить, вправо = назад к списку чатов ----
let touchStartX = 0, touchStartY = 0, touchCurrentX = 0, swipingRow = null, isHorizontalSwipe = false;
...
$('#messages').addEventListener('touchstart', (e) => {
  ...
  if (swipingRow) swipingRow.dataset.wasSwipe = '';
}, { passive: true });
