...
function renderMessage(msg) {
  ...
  row.innerHTML = `
    <div class="bubble">
      <button type="button" class="reply-trigger-btn" title="Ответить">↩</button>
      ${!isOut ? `<span class="sender">${escapeHtml(msg.sender_name || '')}</span>` : ''}
      ${quoteHtml}
      ${bodyHtml}
      <div class="time">${time}${isOut ? `<span class="msg-status${msg.is_read ? ' read' : ''}">${msg.is_read ? '✓✓' : '✓'}</span>` : ''}</div>
    </div>`;
  box.appendChild(row);
  box.scrollTop = box.scrollHeight;

  if (!isOut) unreadIncomingIds.add(msg.id);
}
