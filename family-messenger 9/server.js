...
app.get('/api/users/:id', authMiddleware, asyncRoute(async (req, res) => {
  const user = await get('SELECT id, username, avatar, display_name, last_seen FROM users WHERE id = ?', [req.params.id]);
...
app.get('/api/chats', authMiddleware, asyncRoute(async (req, res) => {
  const chatsRaw = await all(`
    SELECT c.id, c.type, c.title, c.avatar, c.created_by
    FROM chats c
