  ...
  chats.forEach((chat) => {
    ...
    div.innerHTML = `
      ...
      <span class="chat-item-texts">
        <span class="chat-item-title">${escapeHtml(chat.title || 'Чат')}${statusBadges({ verified: chat.otherVerified, is_admin: chat.otherIsAdmin, premium: chat.otherPremium })}</span>
...
async function openChat(chatId) {
  ...
  $('#chat-title').innerHTML = chat ? escapeHtml(chat.title) + statusBadges({ verified: chat.otherVerified, is_admin: chat.otherIsAdmin, premium: chat.otherPremium }) : '';
...
$('#chat-header-info').onclick = async () => {
  ...
  if (me.is_admin) {
    ...
    premiumBtn.onclick = async () => {
      await api(`/api/users/${user.id}/premium`, { method: user.premium ? 'DELETE' : 'POST' });
...
function premiumBadge(user) {
  ...
  return `<span class="premium-badge" data-tooltip="Premium-подписка">
    <svg viewBox="0 0 24 24" width="15" height="15">
      <path fill="#a855f7" d="M12 2l2.6 5.9 6.4.6-4.8 4.3 1.4 6.3L12 15.9 6.4 19.1l1.4-6.3L3 8.5l6.4-.6z"/>
    </svg>
  </span>`;
}
...
$('#settings-btn').onclick = () => {
  ...
  if (me.is_admin) {
    ...
    selfPremiumBtn.onclick = async () => {
      await api(`/api/users/${me.id}/premium`, { method: me.premium ? 'DELETE' : 'POST' });
      await refreshMyProfile();
      selfPremiumBtn.textContent = me.premium ? '🚫 Забрать Premium у себя' : '⭐ Выдать Premium себе';
