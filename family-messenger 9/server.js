...
app.get('/api/me', authMiddleware, asyncRoute(async (req, res) => {
  const user = await get('SELECT id, username, avatar, display_name, bio, phone, birthday, verified, is_admin FROM users WHERE id = ?', [req.user.id]);
  res.json({ user });
}));
...
app.get('/api/users', authMiddleware, asyncRoute(async (req, res) => {
  ...
  const rows = await all('SELECT id, username, avatar, display_name, verified FROM users WHERE username LIKE ? AND id != ? LIMIT 20', [q, req.user.id]);
  res.json(rows);
}));

app.get('/api/users/:id', authMiddleware, asyncRoute(async (req, res) => {
  const user = await get('SELECT id, username, avatar, display_name, last_seen, bio, phone, birthday, verified, is_admin FROM users WHERE id = ?', [req.params.id]);
  ...
  for (const chat of chatsRaw) {
    ...
    if (chat.type === 'private') {
      const other = await get(`
        SELECT u.id, u.username, u.avatar, u.display_name, u.verified, u.is_admin FROM chat_members cm
    ...
    result.push({ ...chat, title, otherUserId, otherAvatar, otherVerified, otherIsAdmin, muted: !!mutedRow, lastMessage: lastMsg || null });
  }
...
app.get('/api/chats/:id/members', authMiddleware, asyncRoute(async (req, res) => {
  ...
  const members = await all(`
    SELECT u.id, u.username, u.avatar, u.display_name, u.verified FROM chat_members cm
    JOIN users u ON u.id = cm.user_id
    WHERE cm.chat_id = ?
  `, [req.params.id]);
  res.json(members);
