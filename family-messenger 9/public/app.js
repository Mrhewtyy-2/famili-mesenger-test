...
function renderMyAvatar() {
  $('#me-username').innerHTML = escapeHtml(me.display_name || me.username) + verifiedBadge(me);
  ...
  chats.forEach((chat) => {
    ...
    div.innerHTML = `
      ...
      <span class="chat-item-texts">
        <span class="chat-item-title">${escapeHtml(chat.title || 'Чат')}${verifiedBadge({ verified: chat.otherVerified, is_admin: chat.otherIsAdmin })}</span>
...
async function openChat(chatId) {
  ...
  $('#chat-title').innerHTML = chat ? escapeHtml(chat.title) + verifiedBadge({ verified: chat.otherVerified, is_admin: chat.otherIsAdmin }) : '';
...
$('#chat-header-info').onclick = async () => {
  ...
  $('#profile-name').innerHTML = escapeHtml(user.display_name || user.username) + verifiedBadge(user);
...
function renderGroupMembers(members) {
  ...
  members.forEach((m) => {
    ...
    row.innerHTML = `
      <span class="avatar-circle"></span>
      <span>${escapeHtml((m.display_name || m.username) + (isMe ? ' (вы)' : ''))}${verifiedBadge(m)}</span>
