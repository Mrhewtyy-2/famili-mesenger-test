...
function buildMessageRow(msg) {
  ...
  if (msg.type === 'location') {
    ...
    bodyHtml = `<video class="video-bubble" src="${msg.media_url}" controls playsinline></video>`;
  } else if (msg.type === 'voice') {
    bodyHtml = `<div class="voice-bubble">🎤<audio src="${msg.media_url}" controls></audio></div>`;
  } else if (msg.type === 'missed_call') {
    bodyHtml = `<div class="missed-call">📵 Пропущенный звонок</div>`;
  } else {
  ...
  const actions = [`<button type="button" class="bubble-action-btn reply-btn" title="Ответить">↩</button>`,
    `<button type="button" class="bubble-action-btn react-btn" title="Реакция">🙂</button>`];
  if (isOut && msg.type === 'text') actions.push(`<button type="button" class="bubble-action-btn edit-btn" title="Редактировать">✏️</button>`);
  if (isOut) actions.push(`<button type="button" class="bubble-action-btn delete-btn" title="Удалить">🗑</button>`);

  row.innerHTML = `
    <div class="bubble">
      <div class="bubble-actions">${actions.join('')}</div>
      ${!isOut ? `<span class="sender">${escapeHtml(msg.sender_name || '')}</span>` : ''}
      ${quoteHtml}
      ${bodyHtml}
      <div class="time">${time}${msg.edited ? '<span class="message-edited-tag">ред.</span>' : ''}${isOut ? `<span class="msg-status${msg.is_read ? ' read' : ''}">${msg.is_read ? '✓✓' : '✓'}</span>` : ''}</div>
...
$('#messages').addEventListener('click', async (e) => {
  ...
  if (replyBtn) return startReply(row.dataset.messageId);

  const editBtn = e.target.closest('.edit-btn');
  if (editBtn) return startEdit(row.dataset.messageId);

  const deleteBtn = e.target.closest('.delete-btn');
  if (deleteBtn) {
    if (!confirm('Удалить это сообщение?')) return;
    await api(`/api/messages/${row.dataset.messageId}`, { method: 'DELETE' });
    return;
  }
  ...
  if (reactBtn) {
    ...
    picker.innerHTML = REACTION_EMOJIS.map((em) => `<button type="button" data-emoji="${em}">${em}</button>`).join('');
    reactBtn.closest('.bubble-actions').appendChild(picker);
    picker.querySelectorAll('button').forEach((btn) => {
      btn.onclick = (ev) => {
        ev.stopPropagation();
        socket.emit('reaction:toggle', { messageId: row.dataset.messageId, emoji: btn.dataset.emoji });
        picker.remove();
      };
    });
    return;
...
document.addEventListener('click', (e) => {
  if (!e.target.closest('.react-btn')) document.querySelectorAll('.reaction-picker').forEach((p) => p.remove());
});
