...
const NON_MENU_TARGETS = 'img, a, audio, video, iframe, .reaction-pill, source';

let lastTouchHandledAt = 0;

// ---- Клик мышью (только десктоп, без тачскрина) — открывает тап-меню сообщения ----
$('#messages').addEventListener('click', (e) => {
  ...
  if (reactionPill) {
    const row = reactionPill.closest('.msg-row');
  ...
  if (e.target.closest(NON_MENU_TARGETS)) return;
  const bubble = e.target.closest('.bubble');
  const row = bubble?.closest('.msg-row');
  if (row) openMessageMenu(row.dataset.messageId);
});
$('#image-lightbox').addEventListener('click', () => $('#image-lightbox').classList.add('hidden'));

// ---- Свайпы/тапы на телефоне: определяем прямо в touchend, не полагаясь на синтетический click ----
let touchStartX = 0, touchStartY = 0, touchCurrentX = 0, touchCurrentY = 0, swipingRow = null, isHorizontalSwipe = false;

$('#messages').addEventListener('touchstart', (e) => {
  touchStartX = touchCurrentX = e.touches[0].clientX;
  touchStartY = touchCurrentY = e.touches[0].clientY;
  swipingRow = e.target.closest('.msg-row');
...
$('#messages').addEventListener('touchmove', (e) => {
  ...
  if (!isHorizontalSwipe && Math.abs(deltaX) > 12 && Math.abs(deltaX) > Math.abs(deltaY) * 1.5) {
    isHorizontalSwipe = true;
  }
  if (isHorizontalSwipe && swipingRow && deltaX < 0) {
    const bubble = swipingRow.querySelector('.bubble');
    const clamped = Math.max(deltaX, -70);
    bubble.style.transform = `translateX(${clamped}px)`;
    swipingRow.classList.toggle('swipe-armed', clamped <= -55);
  }
}, { passive: true });
  ...
  if (isHorizontalSwipe && deltaX <= -55 && swipingRow) {
    ...
    if (target) {
      ...
      if (reactionPill) {
        ...
        $('#image-lightbox').classList.remove('hidden');
      } else if (!target.closest(NON_MENU_TARGETS)) {
